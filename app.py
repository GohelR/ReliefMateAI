# app.py — ReliefMate AI (Streamlit, polished UI + latest OpenAI SDK)
import os
...
st.markdown(
    ...
    .report-row { display:flex; gap:8px; align-items:center; justify-content:space-between; padding:8px 0; border-bottom:1px solid rgba(255,255,255,0.06); }
    .pill { display:inline-block;padding:6px 12px;background:#062634;border-radius:999px;color:#9ff3ff;font-weight:600;font-size:0.9rem; }
    </style>
    """,
...
with st.container():
    ...
    with c2:
        st.markdown(
            "<div class='hero'><strong style='font-size:1.05rem'>Verified relief info — shelters, medical aid & helplines</strong>"
            "<div class='muted' style='margin-top:6px'>Fast, simple and multilingual assistance for people and volunteers during crises.</div></div>",
            unsafe_allow_html=True,
        )
...
with left:
    ...
    st.markdown("<div class='card'>Ask ReliefMate about shelters, medical help or submit a short report.</div>", unsafe_allow_html=True)
    ...
    if user_prompt:
        ...
        if user_prompt.strip().lower().startswith("report"):
            st.chat_message("assistant").write("You can use the **Report an Incident** form on the right.")
        elif not client:
            st.chat_message("assistant").write("⚠️ Chatbot is offline — API key not set. Use Resources or Report form for help.")
        else:
    ...
    if st.session_state.reports:
        for r in reversed(st.session_state.reports[-6:]):
            st.markdown(
                f"<div class='report-row'><div><strong>{r['type']}</strong> • <span class='small'>{r['location']}</span>"
                f"<div class='small'>{r['details'][:90]}</div></div><div class='small'>{r['time'][:19]}</div></div>",
                unsafe_allow_html=True,
            )
    else:
...
with right:
    ...
    if st.button("Example: Nearest shelter (Rajkot)"):
        st.info("Try asking: *Nearest shelter in Rajkot* in the chat.")

    st.markdown("## 📍 Verified Resources")
    st.markdown(
        "<div class='card'><div class='small'>Official Helplines</div><ul>"
        "<li>🚨 National Emergency: 112</li>"
        "<li>🩺 Medical Helpline: 108</li>"
        "</ul></div>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown(f"<div class='small'>Open reports: <strong>{len(st.session_state.reports)}</strong></div>", unsafe_allow_html=True)
    if st.checkbox("Show raw reports (debug)"):
        st.json(st.session_state.reports)

# ---------- Footer ----------
st.markdown("---")
st.caption("ℹ️ For real deployments, connect reports to a DB (Firestore/Supabase/etc.), add verification flows, and NGO dashboards. OpenAI costs money; keep replies short.")
